...
##### Using Custom Format 
This uses the env `DUMP_ARGS=-Fc` to dump a compressed archive.

##### Using Directory Format 
This uses the env `DUMP_ARGS=-Fd` to back up
...
## Backup Location
By default, the image backups the files to a docker volume
or a directory mounted within the image. You can also back up
directly to cloud storage i.e. Minio

### Backing up to S3 bucket
The images utilize [s3cmd](https://s3tools.org/s3cmd) for backing up files to S3 bucket.
...
Restoring files uses the same logic except for S3 backends
where it has to download the `.gz` or `.dir.tar.gz` locally
and then restore them into an empty database.
...
#### Date
Set the env `TARGET_ARCHIVE_DATE`.
...
#### DateTime
Set the env `TARGET_ARCHIVE_DATETIME`.
